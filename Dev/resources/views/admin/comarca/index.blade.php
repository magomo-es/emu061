...
@section('pageTitle', 'Comarques list | Administració emu061 - Emulador de Sistema d\'Emergències 061')
...
<div class="card mb-3">
    ...
    <div class="card-body">
        ...
        <form action="{{ action([App\Http\Controllers\ComarcaController::class, 'index']) }}" method="GET">
            @csrf
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    ...
                    <select class="custom-select" id="srchfilter1" name="srchfilter1">
                        <option value="0" selected>Tots les provincies</option>
                        @foreach ($provinciesAry as $provincia)
                        <option value="{{ $provincia->id }}" {{ ((old('srchfilter1')==$provincia->id)?'selected':'') }}>{{ $provincia->nom }}</option>
                        @endforeach
                    </select>
...
<div class="card">
    ...
    <div class="card-header bg-white">Comarques</div>
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $theobject->id }}</th>
                    <td>{{ $theobject->nom }}</td>
                    <td>{{ $provinciesAry->firstWhere('id', $theobject->provincies_id)->nom }}</td>
                    <td class="text-right">

                        <div class="btn-group" role="group">
                            <form class="m-0 p-0" action="{{ action([App\Http\Controllers\ComarcaController::class, 'edit'], ['theobj' => $theobject->id]) }}">
                                @csrf
                        ...
                        <div class="btn-group ml-1" role="group">
                            ...
                            <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#boxModal"
                             ...
                             data-action="{{ action([App\Http\Controllers\ComarcaController::class, 'destroy'], ['theobj' => $theobject->id]) }}"
